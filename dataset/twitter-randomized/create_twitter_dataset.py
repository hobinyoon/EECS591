...
def generate_dataset(filename, user_ip_map, uuid_server_map, rt_re_set, server_locations):
    ...
    write_entries = []
    read_entries = []
    servers_list = list(server_locations.keys())
    count = 0
    with open(filename, 'rb') as dataset, open(UUID_LIST_FILE, 'wb') as uuid_file:
        for line in dataset:
            ...
            write_target_server = servers_list[count % len(servers_list)]
            if mode == TWITTER_MT:
                ...
                if triplet not in rt_re_set:  # make sure that we don't process the tweet twice
                    ...
                    write_entries.append(LOG_FORMAT % (timestamp, tweet_uid, first_user_ip, 'null', write_target_server, 'WRITE', '201', '0'))
...
def generate_user_ip_map(filename, servers):
    ...
    with open(USER_IP_MAP_FILE, 'wb') as user_ip_map_file, open('access_log.txt', 'wb') as access_log, open(IP_LAT_LONG_MAP, 'wb') as ip_lat_lon_file:
        ...
        for key, value in user_ip_map.iteritems():
            ...
            access_log.write(LOG_FORMAT % (user_count, key, ip_address, 'null', servers_list[user_count % len(servers_list)], 'WRITE', '201', '0'))
            user_count += 1
...
if __name__ == '__main__':
    ...
    generate_dataset(filename, user_ip_map, uuid_server_map, rt_re_set, server_locations)
